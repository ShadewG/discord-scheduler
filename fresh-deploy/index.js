// index.js - Discord Scheduler Bot for Editor reminders
...
/* ─ Schedule definition ──────────────────────────
   ...
   DOW: 1-5  →  Monday‑Friday only (not weekends)
   TZ param ensures times respect Europe/Berlin timezone through DST changes */
let jobs = [
  // Morning schedule
...
function scheduleJob(job) {
  ...
  const scheduledJob = cron.schedule(job.cron, () => ping(job.text), { timezone: TZ });
  activeJobs.set(job.tag, scheduledJob);
  console.log(`Scheduled job: ${job.tag} (${job.cron})`);
  return scheduledJob;
}
...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'test') {
    ...
    await interaction.reply(`✅ Added new reminder "${tag}" scheduled for \`${cronExp}\``);
  }
});
...
client.once('ready', async () => {
  ...
  setTimeout(() => {
    sendMessage('Bot is now online and ready to send reminders! Use /help to see available commands.');
