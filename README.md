...
To export comments outside the bot, run the helper script:

```bash
python scripts/export-frameio-comments.py
```

It reads `FRAMEIO_TOKEN`, `FRAMEIO_ACCOUNT_ID`, `FRAMEIO_PROJECT_ID`, and
`FRAMEIO_ROOT_ASSET_ID` from the environment and writes recent comments to
`frameio_recent_comments.json`.
