...
2. The endpoint accepts JSON or URL-encoded form data with `from`, `subject`
   and `text` fields. Most providers can be configured to send these values.
3. Set `EMAIL_CHANNEL_ID` in your `.env` with the Discord channel that should
   receive the messages (e.g. `1377188959695798332`).
