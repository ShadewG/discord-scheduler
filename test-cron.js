// Simple test script for cron calculation
...
testJobs.forEach(job => {
  try {
    ...
    const nextExecution = getTimeUntilNextExecution(job.cron, TZ);
...
if (process.argv.length > 2) {
  ...
  try {
    const nextExecution = getTimeUntilNextExecution(customCron, TZ);
    if (nextExecution) {
      console.log(`- Next execution: ${nextExecution.formatted}`);
      console.log(`- Time until next: ${nextExecution.formattedTimeLeft}`);
    } else {
